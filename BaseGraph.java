 ...
 * directed and doubly weighted edges connecting those nodes.
 */
public class BaseGraph<NodeType, EdgeType extends Number> {
    ...
    // Each edge contains 2 pieces of data, and two nodes that it connects
    protected class Edge {
        public EdgeType cost; // the cost of this edge
        public EdgeType distance; // the distance of this edge
        public Node predecessor;
        public Node successor;

        public Edge(EdgeType cost, EdgeType distance, Node pred, Node succ) {
            this.cost = cost;
     ...
     * in hat edge to be weight.
     * 
     * @param pred   is the data item contained in the new edge's predecesor
     *               node
     * @param succ   is the data item contained in the new edge's successor node
     * @param distance is the first non-negative data item stored in the new edge
     * @param cost is the second data item stored in the edge
     * @return true if the edge could be inserted or updated, or false if the
     *         pred or succ data are not found in any graph nodes
     */
    public boolean insertEdge(NodeType pred, NodeType succ, EdgeType distance, EdgeType cost) {
        ...
        try {
            ...
            Edge newEdge = new Edge(cost, distance, predNode, succNode);
            this.edgeCount++;
     ...
     * @param pred the data item contained in the source node for the edge
     * @param succ the data item contained in the target node for the edge
     * @return array with two elements, first is cost and second is distance
    ...
    public List<EdgeType> getEdge(NodeType pred, NodeType succ) {
        ArrayList<EdgeType> data = new ArrayList<>();
        data.add(getEdgeHelper(pred, succ).cost);
        data.add(getEdgeHelper(pred, succ).distance);
        return data;
    }
